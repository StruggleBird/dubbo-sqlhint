...
public class RouterProviderFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        try {
            ...
            // 接收到调用的时候以传入的为准
            String sqlrouter = invocation.getAttachment(RouterConsts.ROUTER_KEY);
            if (sqlrouter != null) {
                // 如果路由信息不为空则执行并传递该路由
                RouterContext.put(RouterConsts.ROUTER_KEY, sqlrouter);
            }
            Result result = invoker.invoke(invocation);

            // 返回结果时进行判断，如果当前调用链中包含了路由信息则往回传递该路由
            if (sqlrouter != null) {
                result.getAttachments().put(RouterConsts.ROUTER_KEY, sqlrouter);
            }

            // 如果线程上下文包含了路由信息，则往回传递该路由
            if (RouterContext.containsKey(RouterConsts.ROUTER_KEY)) {
                result.getAttachments().put(RouterConsts.ROUTER_KEY, RouterContext.get(RouterConsts.ROUTER_KEY));
            }
            return result;
